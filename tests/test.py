...
def check(title, path, error=False, stderr_match=None, stdout_match=None):
    ...
    if (error == (p.returncode > 0)
        and (stdout_match is None or re.search(stdout_match, p.stdout.read().decode('utf8')))
        and (stderr_match is None or re.search(stderr_match, p.stderr.read().decode('utf8')))):
        message += 'ok'
    else:
        errors += 1
        message += red('error')
